...
export async function Partners() {
  ...
  let sanityPartners: any[] = [];
  try {
    [section, sanityPartners] = await Promise.all([
      client.fetch(partnersSectionQuery),
      client.fetch(partnersQuery),
    ]);
  } catch {}

  const heading = section?.heading ?? "Created in partnership with industry leaders";

  const logos = sanityPartners.length
    ? sanityPartners.map((p: any) => ({
        _id:        p._id,
        name:       p.name,
